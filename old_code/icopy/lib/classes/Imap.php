...
class Imap {
    ...
    public function getMailbox() {
        ...
        $port = isset($conf['ssl']) && $conf['ssl'] ? '993' : '143';
    ...
    public function getFolderSeparator() {
        $conf = $this->getConf();
        return isset($conf['gmail']) && $conf['gmail'] ? '/' : '.';
    }
    ...
    public function joinFolderPath(array $folderPath, $trim = false) {
        if ($trim) {
            ...
            if (isset($conf['gmail']) && !empty($conf['gmail'])) {
...
//--
    ...
    public function getAllFolders() {
        $subFolders = imap_list($this->getConnection(), $this->getMailbox(), '*');
        if (!empty($subFolders)) {
            ...
            sort($subFolders);
            return array_values($subFolders);
        }
        return false;
    }
//--
    public function getSelFolders($folder, $pattern = '%') {
